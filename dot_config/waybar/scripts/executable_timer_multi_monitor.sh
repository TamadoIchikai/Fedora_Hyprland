...
# - Per-Waybar-instance FIFO + shared timer state across monitors
# - JSON-safe escaping
...
handle_expired_timer() {
  # Multiple Waybar instances can all notice "REM <= 0".
  # This lock ensures only one instance plays sounds, sends notifications,
  # and advances the shared timer state.
  ...
  if [ "$STATE" != "RUNNING" ]; then
    if have_cmd flock; then flock -u 9 2>/dev/null || true; exec 9>&- 2>/dev/null || true; fi
    return 0
  fi

  local ELAPSED=$((NOW - START_TIME))
  local REM=$((SEC_SET - ELAPSED))
...
trigger_update() {
  # Broadcast wake-up pings to every Waybar module instance.
  # timeout prevents a stale FIFO from blocking controller actions forever.
  [ -d "$UPDATE_DIR" ] || return 0

  local f
  for f in "$UPDATE_DIR"/*.fifo; do
    [ -p "$f" ] || continue

    if have_cmd timeout; then
      timeout 0.1s bash -c 'printf "1\n" >"$1"' _ "$f" >/dev/null 2>&1 || rm -f "$f" 2>/dev/null || true
    else
      ( printf '1\n' >"$f" ) >/dev/null 2>&1 &
    fi
  done
...
cleanup() {
  ...
  [ -n "${PIPE_FILE:-}" ] && rm -f "$PIPE_FILE" 2>/dev/null || true
  rmdir "$UPDATE_DIR" 2>/dev/null || true
  exit 0
}
trap cleanup SIGTERM SIGINT EXIT

# Do NOT enforce a singleton daemon here.
# Waybar starts one exec process per visible module/monitor.
# Each process prints JSON to its own Waybar instance, while sharing STATE_FILE.
[ -f "$STATE_FILE" ] || init_state
mkdir -p "$UPDATE_DIR" 2>/dev/null || true
PIPE_FILE="${UPDATE_DIR}/waybar_timer.${PPID}.$$.fifo"
